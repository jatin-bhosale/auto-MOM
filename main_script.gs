...
var mail_list = [
    "Pavan Jangam","pavan.jangam@somaiya.edu",
    "Rinkesh Sante","rinkesh.sante@somaiya.edu",
    "Pratik Gangapurwala","p.gangapurwala@somaiya.edu",
    "Vidit Shah","vidit.shah@somaiya.edu"
  ];
...
function createDocFromForm() {
  ...
  var lst = list(response_data["Email Address"], response_data["Members"]);


  email("all",lst,response_data["Minutes of Meeting prepared by"], filename, pdfFile);
  
}


function list(author, mem){ //for presentees only
  var l = "jatin.bhosale@somaiya.edu",
  l = l + ", " + author;
  for (i = 0; i < mail_list.length; i+=2){
    if(mem.includes(mail_list[i])){
      l = l + ", " + mail_list[i+1];
    }
  }
  return l;
...
/*
function list(author){  // for everyone
  var l = "jatin.bhosale@somaiya.edu";
  l = l + ", " + author;
  for (i = 1; i < mail_list; i+=2){
    l = l + ", " + mail_list[i];
  }
  return l;
}
